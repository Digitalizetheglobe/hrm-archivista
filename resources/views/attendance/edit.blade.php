...
<div class="row">
    ...
    <div class="form-group col-lg-6 col-md-6">
        ...
        {{ Form::hidden('clock_in_latitude', $attendanceEmployee->clock_in_latitude, ['id' => 'clock_in_latitude']) }}
        {{ Form::hidden('clock_in_longitude', $attendanceEmployee->clock_in_longitude, ['id' => 'clock_in_longitude']) }}
        {{ Form::hidden('clock_in_location', $attendanceEmployee->clock_in_location, ['id' => 'clock_in_location']) }}
        <small class="text-muted mt-1" id="clock_in_location_text">
            @if(!empty($attendanceEmployee->clock_in_location))
                📍 {{ $attendanceEmployee->clock_in_location }}
            @endif
        </small>
    </div>
    ...
    <div class="form-group col-lg-6 col-md-6">
        ...
        <!-- Hidden fields for location -->
        {{ Form::hidden('clock_out_latitude', $attendanceEmployee->clock_out_latitude, ['id' => 'clock_out_latitude']) }}
        {{ Form::hidden('clock_out_longitude', $attendanceEmployee->clock_out_longitude, ['id' => 'clock_out_longitude']) }}
        {{ Form::hidden('clock_out_location', $attendanceEmployee->clock_out_location, ['id' => 'clock_out_location']) }}
        <small class="text-muted mt-1" id="clock_out_location_text">
            @if(!empty($attendanceEmployee->clock_out_location))
                📍 {{ $attendanceEmployee->clock_out_location }}
            @endif
        </small>
    </div>
</div>
