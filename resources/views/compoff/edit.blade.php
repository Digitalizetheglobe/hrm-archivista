...
<div class="col-md-12 mt-4">
    <div class="card shadow-lg border-0" style="border-radius: 15px;">
        <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between" style="border-top-left-radius: 15px; border-top-right-radius: 15px;">
            <h5 class="mb-0 text-white font-weight-bold"><i class="ti ti-pencil me-2"></i>{{ __('Edit Comp-Off Entry') }}</h5>
        </div>
        <div class="card-body p-4">
            {{ Form::model($comp_off, ['route' => ['compoff.update', $comp_off->id], 'method' => 'PUT', 'id' => 'compoff-form']) }}
            <div class="row">
                <!-- Branch Selection -->
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2-custom" name="branch_id" id="branch_id" required>
                            ...
                            @foreach ($branches as $id => $name)
                                <option value="{{ $id }}" {{ $selectedBranch == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                        ...
                        <div class="card-body p-3">
                            <div class="form-group mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="dates" id="dates_picker" class="form-control" value="{{ old('dates', $datesString) }}" placeholder="{{ __('Choose one or multiple dates...') }}" required>
                                </div>
                            </div>
            ...
            <div class="modal-footer bg-light p-3 mt-4" style="border-radius: 10px;">
                ...
                <input type="submit" value="{{ __('Update Comp-Off') }}" class="btn btn-primary rounded-pill px-4 shadow">
            </div>
...
$(document).ready(function() {
    ...
    $('#employee_ids option').each(function() {
        allLoadedEmployees.push({
            id: $(this).val(),
            name: $(this).text(),
            department_name: "{{ __('Selected Departments') }}"
        });
    });
    ...
    let selectedDatesList = oldDates ? oldDates.split(',').map(d => d.trim()) : {!! json_encode($dates) !!};
                        ...
                        <div class="employee-card d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted"><i class="ti ti-hierarchy me-1"></i>${employee.department_name || "{{ __('Selected Departments') }}"}</small>
                                </div>
                            </div>
