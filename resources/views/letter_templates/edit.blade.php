...
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block font-weight-400 mb-0">{{ __('Edit Letter Template') }}</h5>
    </div>
@endsection
            ...
            $('#source_letter').change(function() {
                ...
                if (letterName) {
                    if (confirm('Loading letter content will replace the current content. Do you want to continue?')) {
                        ...
                        $(this).val('{{ $letterTemplate->source_letter ?? '' }}');
                    }
                }
            });
        });
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit Letter Template') }}</h6>
                        <a href="{{ route('letter_templates.index') }}" class="btn btn-secondary btn-sm float-right">
                            <i class="fas fa-arrow-left"></i> {{ __('Back') }}
                        </a>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('letter_templates.update', $letterTemplate->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="name" name="name" value="{{ $letterTemplate->name }}" required>
                                        @error('name')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="source_letter" name="source_letter">
                                            ...
                                            @foreach ($letters as $letter)
                                                <option value="{{ $letter }}" {{ ($letterTemplate->source_letter == $letter) ? 'selected' : '' }}>{{ $letter }}</option>
                                            @endforeach
                                        </select>
                                        <small class="text-muted">{{ __('Select a letter to load its content as a starting point (will replace current content)') }}</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="content" name="content" class="form-control" required>{!! $letterTemplate->content !!}</textarea>
                ...
                <div class="card mt-3">
                    <div class="card-header">
                        <h6 class="m-0 font-weight-bold text-warning">{{ __('Template Preview') }}</h6>
                    </div>
                    <div class="card-body">
