    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>{{ __('Employee Invoice') }}</h1>
                <div class="section-header-breadcrumb">
                    <div class="breadcrumb-item active"><a href="#">{{ __('Home') }}</a></div>
                    <div class="breadcrumb-item">{{ __('Employee Invoice') }}</div>
                </div>
            </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped mb-0" id="dataTable1">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($payslip as $invoice)
                                                <tr>
                                                    <td>{{ !empty(\App\Models\Invoice::employee($invoice->employee_id))? \App\Models\Invoice::employee($invoice->employee_id)->name: '' }}
                                                    </td>
                                                    <td>{{ $invoice->salary_month }}</td>
                                                    <td>{{ $invoice->basic_salary }}</td>
                                                    <td>{{ $invoice->net_payble }}</td>
                                                    <td>
                                                        @if ($invoice->status == 1)
                                                    ...
                                                    <td>
                                                        <a href="#"
                                                            data-url="{{ route('invoice.showemployee', $invoice->id) }}"
                                                        ...
                                                        <a href="#"
                                                            data-url="{{ route('invoice.pdf', [$invoice->employee_id, $invoice->salary_month]) }}"
                                                            data-size="md-pdf"
