...
use App\Exports\EmployeeWiseReportExport;
use App\Exports\LeaveExport;
use App\Exports\LeaveReportExport;
use App\Exports\PayrollExport;
use App\Exports\ProjectWiseReportExport;
   ...
   public function projectWiseReport(Request $request)
    {
        ...
        // Main query to get project-wise summary
        $query = TimeSheet::with(['employee', 'project.client'])
            ->selectRaw('
                project_id,
        ...
        // Get all timesheet data for the selected project (if any)
        $timesheetDetails = [];
        $totalCost = 0; // Add this variable to store total cost
        
        if ($request->has('project_id') && $request->project_id) {
            $timesheetQuery = TimeSheet::with(['employee', 'project.client'])
                ->where('project_id', $request->project_id);
                
            if ($request->has('start_date') && $request->has('end_date')) {
                $timesheetQuery->whereBetween('date', [$request->start_date, $request->end_date]);
            }
            
            $timesheetDetails = $timesheetQuery->get();
            
            // Calculate total cost
            foreach ($timesheetDetails as $timesheet) {
                $totalCost += $timesheet->total_time * ($timesheet->employee->hourly_charged ?? 0);
            }
        }

        foreach ($timeSheets as $sheet) {
            $employeeIds = explode(',', $sheet->employee_ids);
            $employees = Employee::whereIn('id', $employeeIds)->get();
            
            // Calculate labor cost (time * hourly rate for each employee)
            $totalEmployeeCost = 0;
            foreach ($employees as $employee) {
                $employeeHours = TimeSheet::where('project_id', $sheet->project_id)
                    ->where('employee_id', $employee->id);
                    
                if ($request->has('start_date') && $request->has('end_date')) {
                    $employeeHours->whereBetween('date', [$request->start_date, $request->end_date]);
                }
                
                $employeeHours = $employeeHours->sum('total_time');
                $totalEmployeeCost = $employeeHours * $employee->hourly_charged;
            }

            $projectData = [
                'client_name' => $sheet->project->client->name ?? '',
                'project_name' => $sheet->project->name ?? '',
                'total_hours' => $sheet->total_hours,
                'employees' => $employees,
                'total_expense' => $sheet->total_expense,
                'total_employee_cost' => $totalEmployeeCost,
                'final_project_cost' =>  $totalEmployeeCost,
                'descriptions' => TimeSheet::where('project_id', $sheet->project_id)
                    ->pluck('narration')
                    ->unique()
                    ->implode(' | ')
            ];
            ...
            // Store data for selected project
            if ($request->has('project_id') && $request->project_id == $sheet->project_id) {
                $selectedProjectData = [
                    'total_hours' => $sheet->total_hours,
                    'total_employees' => count($employeeIds),
                    'total_expense' => $sheet->total_expense,
                    'final_project_cost' => $sheet->total_expense + $totalEmployeeCost
                ];
            }
        }

        // Calculate totals if no specific project is selected
        if (!$request->has('project_id') || !$request->project_id) {
            $selectedProjectData = [
                'total_hours' => array_sum(array_column($projects, 'total_hours')),
                'total_employees' => count(array_unique(array_merge(...array_map(function($project) {
                    return $project['employees']->pluck('id')->toArray();
                }, $projects)))),
                'total_expense' => array_sum(array_column($projects, 'total_expense')),
                'final_project_cost' => array_sum(array_column($projects, 'final_project_cost'))
            ];
        }

        return view('report.project-wise', compact(
            'projects', 
            'allProjects', 
            'selectedProjectData',
            'timesheetDetails',
            'totalCost' // Pass the total cost to the view
        ));
    }

    public function getProjects(Request $request)
    {
        $projects = Project::where('client_id', $request->client_id)->pluck('name', 'id');
        return response()->json($projects);
    }

    public function projectWiseExport(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            // Get filtered timesheet details (same logic as projectWiseReport)
            $timesheetDetails = collect();
            $project = null;
            $projectData = [
                'total_employees' => 0,
                'total_hours' => 0,
            ...
            if ($request->has('project_id') && $request->project_id) {
                ...
                $employeeIds = $timesheetDetails->pluck('employee_id')->unique()->toArray();
                $projectData['total_employees'] = count($employeeIds);
                $projectData['total_hours'] = $timesheetDetails->sum('total_time');
                $projectData['total_expense'] = $timesheetDetails->sum('expense');
                ...
                $projectData['total_cost'] = $totalCost;
            }
            
            $name = 'project_wise_report_' . date('Y-m-d_H-i-s') . '.xlsx';
            $data = \Excel::download(new ProjectWiseReportExport(
                $timesheetDetails, 
                $project, 
                $projectData,
...
{
    \Log::debug('EmployeeWiseReport Request:', $request->all());
    ...
    // If branch is selected, get departments
    if ($request->has('branch_id') && $request->branch_id) {
        $departments = Department::where('branch_id', $request->branch_id)->get();
    }

    // If department is selected, get employees
    if ($request->has('department_id') && $request->department_id) {
        $employees = Employee::where('department_id', $request->department_id)->get();
    }

    // If employee is selected, process the report
    if ($request->has('employee_id') && $request->employee_id) {
        $employee = Employee::findOrFail($request->employee_id);
        
        // Get timesheets for the selected employee
        $timesheetQuery = TimeSheet::with(['project.client'])
            ->where('employee_id', $employee->user_id) // Assuming user_id is the correct foreign key
            ->select('time_sheets.*');
            
        if ($request->has('start_date') && $request->has('end_date')) {
            $timesheetQuery->whereBetween('time_sheets.date', [$request->start_date, $request->end_date]);
        }
        
        // Exclude leave projects (case-insensitive)
        $timesheetQuery->whereHas('project', function($query) {
            $query->whereRaw('LOWER(project_name) NOT LIKE ?', ['%leave%']);
        });
        
        $timesheetDetails = $timesheetQuery->get();
        
        // Calculate report data
        $reportData['total_projects'] = $timesheetDetails->groupBy('project_id')->count();
        $reportData['total_hours'] = $timesheetDetails->sum('total_time');
        $reportData['total_cost'] = $reportData['total_hours'] * $employee->hourly_charged;
    }

    return view('report.employee-wise', compact(
        'branches',
        'departments',
        'employees',
        'employee', // This contains the hourly_charged value
        'timesheetDetails',
        'reportData'
    ));
}

    public function employeeWiseExport(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            // Get filtered timesheet details (same logic as employeeWiseReport)
            $timesheetDetails = collect();
            $employee = null;
            $reportData = [
                'total_projects' => 0,
            ...
            if ($request->has('employee_id') && $request->employee_id) {
                ...
                $timesheetQuery->whereHas('project', function($query) {
                    $query->where('project_name', 'not like', '%leave%');
                });
                ...
                $reportData['total_projects'] = $timesheetDetails->groupBy('project_id')->count();
                $reportData['total_hours'] = $timesheetDetails->sum('total_time');
                $reportData['total_cost'] = $reportData['total_hours'] * ($employee->hourly_charged ?? 0);
            }
            
            $name = 'employee_wise_report_' . date('Y-m-d_H-i-s') . '.xlsx';
            $data = \Excel::download(new EmployeeWiseReportExport(
                $timesheetDetails, 
                $employee, 
                $reportData,
                $request->start_date ?? null,
    ...
    public function branchWiseReport(Request $request)
    {
        $branches = Branch::all();
        ...
        // Get all timesheet data for the selected branch (if any)
        $timesheetDetails = [];
        $totalCost = 0;
        ...
        if ($request->has('branch_id') && $request->branch_id) {
            ...
            // Calculate summary data
            $employeeIds = $timesheetDetails->pluck('employee_id')->unique()->toArray();
            $selectedBranchData['total_employees'] = count($employeeIds);
            $selectedBranchData['total_hours'] = $timesheetDetails->sum('total_time');
            $selectedBranchData['total_expense'] = $timesheetDetails->sum('expense');
            
            // Calculate total cost
            foreach ($timesheetDetails as $timesheet) {
                $totalCost += $timesheet->total_time * ($timesheet->employee->hourly_charged ?? 0);
            }
            $selectedBranchData['total_cost'] = $totalCost;
        }

        return view('report.branch-wise', compact(
            'branches', 
            'selectedBranchData',
            'timesheetDetails',
            'totalCost'
    ...
    public function departmentWiseReport(Request $request)
    {
        ...
        // Get all timesheet data for the selected department (if any)
        $timesheetDetails = [];
        $totalCost = 0;
        ...
        if ($request->has('department_id') && $request->department_id) {
            ...
            // Calculate summary data
            $employeeIds = $timesheetDetails->pluck('employee_id')->unique()->toArray();
            $selectedDepartmentData['total_employees'] = count($employeeIds);
            $selectedDepartmentData['total_hours'] = $timesheetDetails->sum('total_time');
            $selectedDepartmentData['total_expense'] = $timesheetDetails->sum('expense');
            
            // Calculate total cost
            foreach ($timesheetDetails as $timesheet) {
                $totalCost += $timesheet->total_time * ($timesheet->employee->hourly_charged ?? 0);
            }
            $selectedDepartmentData['total_cost'] = $totalCost;
        }

        return view('report.department-wise', compact(
            'branches', 
            'departments',
            'department',
            'selectedDepartmentData',
    ...
    {
        if (\Auth::user()->can('Manage Report')) {
            // Get filtered timesheet details (same logic as branchWiseReport)
            $timesheetDetails = collect();
            $branch = null;
            $branchData = [
                'total_employees' => 0,
            ...
            if ($request->has('branch_id') && $request->branch_id) {
                ...
                // Calculate summary data
                $employeeIds = $timesheetDetails->pluck('employee_id')->unique()->toArray();
                $branchData['total_employees'] = count($employeeIds);
                $branchData['total_hours'] = $timesheetDetails->sum('total_time');
                $branchData['total_expense'] = $timesheetDetails->sum('expense');
                ...
                $branchData['total_cost'] = $totalCost;
            }
            
            $name = 'branch_wise_report_' . date('Y-m-d_H-i-s') . '.xlsx';
            
            // Create a simple export similar to ProjectWiseReportExport
            $data = \Excel::download(new \App\Exports\BranchWiseReportExport(
                $timesheetDetails, 
                $branch, 
                $branchData,
                $request->start_date ?? null,
    ...
    public function departmentWiseExport(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            // Get filtered timesheet details (same logic as departmentWiseReport)
            $timesheetDetails = collect();
            $department = null;
            $departmentData = [
                'total_employees' => 0,
            ...
            if ($request->has('department_id') && $request->department_id) {
                ...
                // Calculate summary data
                $employeeIds = $timesheetDetails->pluck('employee_id')->unique()->toArray();
                $departmentData['total_employees'] = count($employeeIds);
                $departmentData['total_hours'] = $timesheetDetails->sum('total_time');
                $departmentData['total_expense'] = $timesheetDetails->sum('expense');
                ...
                $departmentData['total_cost'] = $totalCost;
            }
            
            $name = 'department_wise_report_' . date('Y-m-d_H-i-s') . '.xlsx';
            
            // Create a simple export similar to ProjectWiseReportExport
            $data = \Excel::download(new \App\Exports\DepartmentWiseReportExport(
                $timesheetDetails, 
                $department, 
                $departmentData,
                $request->start_date ?? null,
