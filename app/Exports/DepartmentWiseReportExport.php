...
class DepartmentWiseReportExport implements FromArray, WithHeadings, WithStyles, WithColumnWidths, WithTitle
{
    ...
    public function __construct($timesheetDetails = null, $department = null, $departmentData = null, $startDate = null, $endDate = null)
    {
    ...
    public function array(): array
    {
        ...
        // Add title section
        $data[] = ['DEPARTMENT WISE REPORT', '', '', '', '', '', '', ''];
        $data[] = ['', '', '', '', '', '', '', ''];
        
        if ($this->department) {
            $data[] = ['Department Name:', $this->department->name ?? '', '', '', '', '', '', ''];
        ...
        $data[] = ['SUMMARY', '', '', '', '', '', '', ''];
        $data[] = ['Total Employees:', $this->departmentData['total_employees'] ?? 0, '', '', '', '', '', ''];
        $data[] = ['Total Time Spent:', number_format($this->departmentData['total_hours'] ?? 0, 2) . ' hrs', '', '', '', '', '', ''];
        $data[] = ['Total Expense:', number_format($this->departmentData['total_expense'] ?? 0, 2), '', '', '', '', '', ''];
        $data[] = ['Total Cost:', number_format($this->departmentData['total_cost'] ?? 0, 2), '', '', '', '', '', ''];
        $data[] = ['', '', '', '', '', '', '', ''];
        $data[] = ['', '', '', '', '', '', '', ''];
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if ($this->department && $this->startDate && $this->endDate) {
        ...
        // Style for department info section (if department exists)
        if ($this->department) {
            $infoStartRow = 3;
