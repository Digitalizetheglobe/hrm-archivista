...
class Utility extends Model
{
    ...
    public static function employeePayslipDetail($employeeId, $month)
    {
        ...
        foreach ($earning['allowance'] as $earn) {
            // allowance is now stored as a numeric value, not JSON
        ...
        foreach ($earning['commission'] as $earn) {
            // commission is now stored as a numeric value, not JSON
        ...
        foreach ($earning['otherPayment'] as $earn) {
            // other_payment is now stored as a numeric value, not JSON
        ...
        foreach ($earning['overTime'] as $earn) {
            // overtime is now stored as a numeric value, not JSON
            $ot += $earn->overtime ?? 0;
        }
        ...
        foreach ($deduction['loan'] as $loan) {
            // loan is now stored as a numeric value, not JSON
        ...
        foreach ($deduction['deduction'] as $deductions) {
            // saturation_deduction is now stored as a numeric value, not JSON
